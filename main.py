...
# ---------------------------
# Networking / Data helpers (robust)
# ---------------------------
...
class InvestmentApp:
    ...
    def _build_ui(self):
        ...
        self.period_combo.set("1y"); self.period_combo.pack(side=tk.LEFT, padx=4)
        ...
        self.interval_combo.set("1d"); self.interval_combo.pack(side=tk.LEFT, padx=4)
        ttk.Button(chart_controls, text="Show Chart", command=lambda: self.show_chart(candles=False)).pack(side=tk.LEFT, padx=6)
        ttk.Button(chart_controls, text="Show Candles", command=lambda: self.show_chart(candles=True)).pack(side=tk.LEFT, padx=6)
    ...
    def _fetch_and_update_live(self, ticker):
        ...
        if price is None:
            ...
            self.root.after(0, lambda: (self._set_status("Failed to fetch price"), messagebox.showerror("Error", f"Price not available for {ticker}")))
            return
    ...
    def load_market(self):
        ...
        def bg():
            ...
            self.root.after(0, lambda: (self.market_text.delete("1.0", tk.END), self.market_text.insert(tk.END, "\n".join(output)), self._set_status("Market overview loaded")))
    ...
    def _fetch_news_thread(self, ticker):
        ...
        def ui():
            ...
            if not items:
                self.news_list.insert(tk.END, "No news found.")
            for n in items:
                title = n.get("title") or "News"
                display = title if len(title) < 120 else title[:120] + "..."
                self.news_list.insert(tk.END, display)
            self.news_list.links = [n.get("link") for n in items]
            self._set_status("News loaded")
    ...
    def open_selected_news(self, evt):
        ...
        if not sel: return
        idx = sel[0]
        link = getattr(self.news_list, "links", [None]*100)[idx]
        if link:
    ...
    # Charting
    # -------------------------
    def show_chart(self, candles=False):
        ticker = self.ticker_entry.var.get().upper().strip()
        if not ticker:
            messagebox.showerror("Error", "Enter a ticker")
            return
        ...
        threading.Thread(target=self._fetch_and_plot, args=(ticker, period, interval, candles), daemon=True).start()

    def _fetch_and_plot(self, ticker, period, interval, candles):
        df = fetch_history(ticker, period=period, interval=interval)
        if df is None or df.empty:
            self.root.after(0, lambda: (messagebox.showerror("Error", "No historical data"), self._set_status("Failed to load history")))
            return
        self.root.after(0, lambda: self._plot_dataframe(ticker, df, candles))
    ...
    def _plot_dataframe(self, ticker, df, candles):
        self._clear_chart()
        self._chart_fig = plt.Figure(figsize=(9,5))
        ax = self._chart_fig.add_subplot(111)
        try:
            if candles and HAVE_MPF:
                # mplfinance expects OHLCV DF with DatetimeIndex
                try:
                    mpf.plot(df, type='candle', volume=True, ax=ax, show_nontrading=False)
                except Exception:
    ...
    def open_portfolio_dialog(self):
        ...
        ttk.Button(top, text="Refresh", command=self._refresh_port_ui).pack(side=tk.RIGHT)
        ...
        ttk.Button(ops, text="Export", command=lambda: save_json("portfolio_export.json", state["portfolio"])).pack(side=tk.RIGHT)
    ...
    def _refresh_port_ui(self):
        ...
        try:
            ...
            for t,v in p.get("positions",{}).items():
                self.pos_list.insert(tk.END, f"{t}  qty:{v['qty']}  avg:${v['avg']:,.2f}")
        except Exception:
            pass
